...
@section('pagetitle') Item | Customer @endsection
...
@section('validation')
	<table>
		...
		<tr>
			...
			<td>
				 @if($item->availability=="Yes" && $item->status=="open") <a href="{{route('customerItem.addToCart', ['itemID' => $item->itemID])}}">Add to cart</a>  
				 @elseif($item->availability=="Yes" && $item->status=="close")
				 Restaurant Closed
				 @endif
				 <a href="{{route('customerView.restaurantDetails', ['userID' => $item->userID])}}">View Restaurant Details</a>
			</td>
		</tr>
